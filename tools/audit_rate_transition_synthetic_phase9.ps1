...
$projectPath = Join-Path $runtimeRoot 'tests\RateTransitionSyntheticNodeAudit\RateTransitionSyntheticNodeAudit.vcxproj'
$workspaceModule = Join-Path $workspaceHome 'tools\FlightEnvWorkspaceConfig.psm1'
$reportRoot = Join-Path $workspaceHome '_local_artifacts\platform-runtime\temporal-multirate-phase9'
$reportPath = Join-Path $reportRoot 'phase9_rate_transition_synthetic_audit.json'
...
if (-not (Test-Path -LiteralPath $projectPath -PathType Leaf)) {
    throw "RateTransitionSyntheticNode audit project not found: $projectPath"
}
...
Write-Host 'FlightEnv rate transition synthetic node Phase9 audit'
...
if ($LASTEXITCODE -ne 0) {
    throw "MSBuild failed for RateTransitionSyntheticNodeAudit with exit code $LASTEXITCODE"
}

$exe = Join-Path $WorkspaceRoot "$Platform\$Configuration\RateTransitionSyntheticNodeAudit.exe"
if (-not (Test-Path -LiteralPath $exe -PathType Leaf)) {
    throw "RateTransitionSyntheticNodeAudit executable not found after build: $exe"
}

New-Item -ItemType Directory -Force -Path $reportRoot | Out-Null
& $exe $reportPath
if ($LASTEXITCODE -ne 0) {
    throw "RateTransitionSyntheticNodeAudit failed with exit code $LASTEXITCODE. Report: $reportPath"
}

$report = Get-Content -LiteralPath $reportPath -Encoding UTF8 -Raw | ConvertFrom-Json
if ($report.status -ne 'passed') {
    throw "RateTransitionSyntheticNodeAudit report did not pass: $reportPath"
}
if ([int]$report.summary.failure_count -ne 0) {
    throw "RateTransitionSyntheticNodeAudit reported failures: $($report.summary.failure_count)"
}

Write-Host "Rate transition synthetic node Phase9 audit passed. Report: $reportPath"
