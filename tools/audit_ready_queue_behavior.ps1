...
$projectPath = Join-Path $runtimeRoot 'tests\ReadyQueueBehaviorAudit\ReadyQueueBehaviorAudit.vcxproj'
...
$reportPath = Join-Path $reportRoot 'ready_queue_behavior_audit.json'
...
if (-not (Test-Path -LiteralPath $projectPath -PathType Leaf)) {
    throw "ReadyQueue behavior audit project not found: $projectPath"
}
...
Write-Host 'FlightEnv ReadyQueue behavior audit'
...
if ($LASTEXITCODE -ne 0) {
    throw "MSBuild failed for ReadyQueueBehaviorAudit with exit code $LASTEXITCODE"
}

$exe = Join-Path $WorkspaceRoot "$Platform\$Configuration\ReadyQueueBehaviorAudit.exe"
if (-not (Test-Path -LiteralPath $exe -PathType Leaf)) {
    throw "ReadyQueueBehaviorAudit executable not found after build: $exe"
}

New-Item -ItemType Directory -Force -Path $reportRoot | Out-Null
& $exe $reportPath
if ($LASTEXITCODE -ne 0) {
    throw "ReadyQueueBehaviorAudit failed with exit code $LASTEXITCODE. Report: $reportPath"
}

Write-Host "ReadyQueue behavior audit passed. Report: $reportPath"
